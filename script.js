...
// ============================================================
// GLOBAL: clickAiOption — must be defined OUTSIDE initISchoolApp
// so it's available immediately when static HTML buttons are clicked.
// Root cause fix: avoid .closest(".p-3.5") because Tailwind class "p-3.5"
// contains a dot, which is a special character in CSS selectors and
// causes closest() to fail silently.
// ============================================================
window.clickAiOption = function(btn) {
  // Walk up DOM tree to find the container with data-explanation attribute
  var container = null;
  var el = btn.parentElement;
  while (el) {
    if (el.hasAttribute("data-explanation")) { container = el; break; }
    el = el.parentElement;
  }
  if (!container) container = document.getElementById("ai-simulator-result");
  if (!container) return;
  ...
  var exp = container.getAttribute("data-explanation") || "Tidak ada penjelasan.";

  // Style all option buttons
  var optBtns = container.querySelectorAll(".ai-opt-btn");
  optBtns.forEach(function(b) {
    b.style.pointerEvents = "none";
    b.disabled = true;
    b.style.cursor = "default";
    var isThisCorrect = b.getAttribute("data-correct") === "true";
    var icon = b.querySelector("i");
    if (isThisCorrect) {
      b.style.background = "rgba(16,185,129,0.15)";
      b.style.borderColor = "rgba(16,185,129,0.5)";
      b.style.color = "#10b981";
      if (icon) { icon.className = "fas fa-check-circle"; icon.style.color = "#10b981"; icon.style.display = "inline"; }
    } else if (b === btn) {
      b.style.background = "rgba(239,68,68,0.15)";
      b.style.borderColor = "rgba(239,68,68,0.5)";
      b.style.color = "#ef4444";
      if (icon) { icon.className = "fas fa-times-circle"; icon.style.color = "#ef4444"; icon.style.display = "inline"; }
    } else {
      b.style.opacity = "0.4";
    }
  });
  ...
  // Show feedback
  var feedbackEl = container.querySelector(".ai-sim-feedback") || container.querySelector("#ai-sim-feedback");
  if (!feedbackEl) {
    feedbackEl = document.createElement("p");
    feedbackEl.className = "ai-sim-feedback";
    feedbackEl.style.cssText = "font-size:11px;font-weight:600;margin-top:10px;padding:9px 12px;border-radius:7px;border:1px solid;display:none;";
  ...
  if (isCorrect) {
    ...
    feedbackEl.innerHTML = '<i class="fas fa-check-circle" style="margin-right:6px"></i><strong>Benar!</strong> ' + exp;
  } else {
    feedbackEl.style.background = "rgba(239,68,68,0.12)";
    feedbackEl.style.borderColor = "rgba(239,68,68,0.4)";
    feedbackEl.style.color = "#ef4444";
    feedbackEl.innerHTML = '<i class="fas fa-times-circle" style="margin-right:6px"></i><strong>Salah!</strong> ' + exp;
  }
...
function initISchoolApp() {
  ...
  window.clickAiOption = function(btn) {
    // Walk up the DOM to find the question container (avoid .p-3.5 selector — dot in classname breaks CSS selector)
    let container = null;
    ...
    // Already answered — prevent double-click
    if (container.dataset.answered === "true") return;
    container.dataset.answered = "true";

    const isCorrect = btn.getAttribute("data-correct") === "true";

    // Disable & style all option buttons
    ...
    optBtns.forEach(function(b) {
      ...
      const icon = b.querySelector("i.fa-check, i.fa-times, i");
      const isThisCorrect = b.getAttribute("data-correct") === "true";
    ...
    let feedbackEl = container.querySelector(".ai-sim-feedback") || container.querySelector("#ai-sim-feedback");
    if (!feedbackEl) {
      // create one dynamically if it doesn't exist yet
      feedbackEl = document.createElement("p");
      feedbackEl.className = "ai-sim-feedback";
      feedbackEl.style.cssText = "font-size:11px;font-weight:600;margin-top:8px;padding:8px 10px;border-radius:6px;border:1px solid;";
      container.appendChild(feedbackEl);
    }

    const exp = container.getAttribute("data-explanation") || "Tidak ada penjelasan.";
    if (isCorrect) {
      feedbackEl.style.background = "rgba(16,185,129,0.12)";
      feedbackEl.style.borderColor = "rgba(16,185,129,0.4)";
      feedbackEl.style.color = "#10b981";
      feedbackEl.innerHTML = '<i class="fas fa-check-circle" style="margin-right:5px"></i><strong>Benar!</strong> ' + exp;
    } else {
      feedbackEl.style.background = "rgba(239,68,68,0.12)";
      feedbackEl.style.borderColor = "rgba(239,68,68,0.4)";
      feedbackEl.style.color = "#ef4444";
      feedbackEl.innerHTML = '<i class="fas fa-times-circle" style="margin-right:5px"></i><strong>Salah!</strong> ' + exp;
    }
    feedbackEl.style.display = "block";
  };

  window.generateAiExam = async function() {
    ...
    try {
      const aiQuestionsDb = window.aiQuestionsDb;
      if (!aiQuestionsDb) throw new Error("Database soal tidak ditemukan");
      ...
      // Choose a random question out of the available ones!
      ...
      console.error("Gagal mengambil data soal AI:", error);
      btn.disabled = false;
      btn.style.pointerEvents = "auto";
      btn.innerHTML = `<i class="fas fa-wand-magic-sparkles"></i> Coba Lagi`;
      if (resultContainer) {
        resultContainer.innerHTML = `<p class="text-red-500 font-bold text-xs">Gagal memuat soal dari API. Pastikan soal_ai.json tersedia.</p>`;
      }
    }
  };
